...
class PredictionNetwork(nn.Module):
    def __init__(
        ...
        - Joint policy head (`fc_policy_joint`): takes only the agent's hidden state; used for pure execution (no MCTS).
        - Agent-specific policy head (`fc_policy_agent`): conditioned on hidden state, agent ID, and prefix; used only during centralized MCTS planning.
        ...
        full_support_size: int
            dim of value output
        prefix_input_dim: int
            dim of the prefix input (e.g., one-hot or marginals)
        prefix_emb_dim: int
            dim of the learned prefix embedding
        agent_emb_dim: int
            dim of the agent ID embedding
        """
        super().__init__()
        ...
        self.fc_value = mlp(
            hidden_state_size * num_agents,
            fc_value_layers,
            full_support_size,
            use_value_out=True
        )

        self.fc_policy_joint = mlp(
            hidden_state_size,
            fc_policy_layers,
            action_space_size,
            use_value_out=True
        ...
        self.agent_id_emb = nn.Embedding(num_agents, agent_emb_dim)
        self.prefix_proj = nn.Linear(prefix_input_dim, prefix_emb_dim)

        policy_in_dim = hidden_state_size + agent_emb_dim + prefix_emb_dim
        self.fc_policy_agent = mlp(
            policy_in_dim,
            fc_policy_layers,
            action_space_size,
            use_value_out=True
    ...
    def forward(self, x):
        ...
        policy_logit = self.fc_policy_joint(
            x.reshape(-1, self.hidden_state_size)
        ).reshape(-1, self.num_agents, self.action_space_size)
    ...
    def forward_agent(
        ...
        """
        Forward pass for a single agent with conditioning prefix.

        hidden_states: Tensor of shape [B, N, H]
        agent_idx:       LongTensor of shape [B]
        prefix:          Tensor of shape [B, prefix_input_dim]
        Returns:
            policy_logits: [B, A]
            value:         [B, 1]
        """
        ...
        h_i = hidden_states[torch.arange(B), agent_idx]  # Extract per-agent hidden: [B, H]

        a_emb = self.agent_id_emb(agent_idx) # Embed agent ID: [B, agent_emb_dim]
        p_emb = F.relu(self.prefix_proj(prefix)) # Prefix embedding: [B, prefix_emb_dim]

        inp = torch.cat([h_i, a_emb, p_emb], dim=-1)  # [B, H+agent_emb+prefix_emb]

        policy_logits = self.fc_policy_agent(inp)

        value = self.fc_value(hidden_states.view(B, -1))
...
class MAMuZeroNet(BaseNet):
    ...
    def initial_inference(self, observation: torch.Tensor, agent_idx: torch.LongTensor = None, prefix: torch.LongTensor = None) -> NetworkOutput:
        ...
        if agent_idx is None:
            ...
            policy_logit, value = self.prediction.forward_agent(
    ...
    def recurrent_inference(self, hidden_state: HiddenState, action: Action, agent_idx: torch.LongTensor = None, prefix: torch.LongTensor = None) -> NetworkOutput:
